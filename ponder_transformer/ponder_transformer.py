...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):#x:torch.Size([1, 512, 512])
        n, h, device = x.shape[1], self.heads, x.device 
        qkv = self.to_qkv(x).chunk(3, dim=-1) #q,k,vの塊のリスト作成 torch.Size([1, 512, 512])×3
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), qkv)#torch.Size([1, 8, 512, 64])
...
class PonderTransformer(nn.Module):
    ...
    def forward(self, x, *, labels=None, mask=None):
        ...
        x = self.token_emb(x)#torch.Size([1, 512])->torch.Size([1, 512, 512])
...
def main():
    ...
    x = torch.randint(0, 20000, (1, 512))#torch.Size([1, 512])
    y = torch.randint(0, 20000, (1, 512))#torch.Size([1, 512])
